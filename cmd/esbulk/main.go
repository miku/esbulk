...
var (
	...
	apiKey             = flag.String("apikey", "", "set the encoded ES api key (mutually exclusive with -u)")
...
func main() {
	...
	if len(*apiKey) > 0 && len(*user) > 0 {
		log.Fatal("username:password and apikey cannot be used simultaneously")
	}
